...
public class BSP : MonoBehaviour
{
    ...
    void Divide(Node tree,int n,int check)
    {
        ...
        if (tree.nodeRect.width >= tree.nodeRect.height) //가로가 더 길었던 경우에는 좌 우로 나누게 될 것이며, 이 경우에는 세로 길이는 변하지 않는다.
        {
            ...
            if(n == maximumDepth-1)
            {
                for(int i = tree.nodeRect.y; i<=tree.nodeRect.y + tree.nodeRect.height ; i++)
                {
                    Instantiate(top_ground,new Vector2(tree.nodeRect.x + split-(mapSize.x/2), i-(mapSize.y/2)),Quaternion.identity); // 생성할시 맵사이즈 절반을 뺴주지 않으면 원치않은 위치에 생성
        ...
        else
        {
            ...
            if(n == maximumDepth-1)
            {
                for(int i = tree.nodeRect.x; i<=tree.nodeRect.x + tree.nodeRect.width;i++ )
                {
